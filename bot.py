...
Задавай свой вопрос, но готовься получить пизды вместо ответа!"""
    ...
    async def send_random_photo(self, update: Update, context: CallbackContext) -> None:
        """Отправляет случайное фото"""
        try:
            random_photo = random.choice(PHOTO_PATHS)
            if os.path.exists(random_photo):
                with open(random_photo, 'rb') as photo:
                    await update.message.reply_photo(photo=photo, caption="💀 Держи фоточку, братан!")
            else:
                logging.warning(f"Фото {random_photo} не найдено!")
...
"Все эти люди, которые ищут смысл, пиздец, они как мухи на ебучей медовой банке — летают, жужжат, а толку ноль."""
            ...
            # Отправляем случайное фото после ответа
